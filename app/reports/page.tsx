      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="card">
          <div className="px-5 py-3 border-b border-gray-50 flex items-center gap-2 flex-wrap">
            {FILTERS.map(f => (
              <Link key={f.value}
                    href={f.value === 'all' ? '/reports' : '/reports?status=' + f.value}
          ...
          {reports.length === 0 ? (
            ...
            <table className="w-full">
              <thead><tr>
                ...
                <th className="th text-center">Lignes</th>
              ...
              <tbody>
                {reports.map((r: any) => (
                  <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                    <td className="td">
                      <Link href={"/reports/"+r.id} className="text-[#4f6ef7] font-medium hover:underline">{r.title}</Link>
                    </td>
                    <td className="td text-gray-400">
                      {formatDateShort(r.period_start)}{r.period_end && r.period_end !== r.period_start ? ' – ' + formatDateShort(r.period_end) : ''}
                    </td>
