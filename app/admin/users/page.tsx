...
export default async function AdminUsersPage() {
  ...
  return (
    <Shell pendingCount={pendingCount??0}>
      <Header title="Utilisateurs" subtitle={users.length+' utilisateur'+(users.length!==1?'s':'')} />
      <div className="flex-1 overflow-y-auto p-6">
        <div className="card">
          <table className="w-full">
            <thead><tr>
              <th className="th">Utilisateur</th><th className="th">Email</th>
              <th className="th">Rôles</th><th className="th">Valideur</th><th className="th">Statut</th>
            </tr></thead>
            <tbody>
              {users.map((u:any) => {
                  ...
                  <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                    <td className="td">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0"
                             style={{ background:'rgba(245,158,11,.15)', color:'#92400e' }}>{ini}</div>
                    ...
                    <td className="td">
                      <div className="flex gap-1 flex-wrap">
                        {roles.map(r => <span key={r} className={RC[r]??'badge badge-draft'}>{RL[r]??r}</span>)}
                      </div>
                    </td>
